...
const PromptCard = ({post, handleTagClick, handleEdit, handleDelete}) => {
  ...
  return (
    <div className="prompt_card relative">
      ...
      <div className="copy_btn absolute top-2 right-2" onClick={handleCopy}>
        <Image 
      ...
      <p className="font-inter text-sm blue_gradient cursor-pointer" onClick={()=> handleTagClick && handleTagClick(post.tag)}>{post.tag}</p>
      {
        session?.user?.id === post.creator._id && pathName === '/profile' && (
          <div className="w-full flex justify-center gap-2 mt-4">
            <p 
              className="font-inter font-semibold text-sm text-green-500 cursor-pointer hover:text-black"
              onClick={handleEdit}
            >
              Edit
            </p>
            <p 
              className="font-inter font-bold text-sm text-red-900 cursor-pointer hover:text-black"
